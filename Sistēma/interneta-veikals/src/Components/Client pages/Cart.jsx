...
const Cart = () => {
  ...
  //Removes emtpy elementrs from the array, causes an error if not filtered.
      ...
      {localStorage.length > 0 ? (
        <>
                        ...
                        <TableRow key={index}>
                          ...
                          <TableCell>
                            <ButtonGroup variant="contained">
                              <Button onClick={updateFieldChanged(index)} value="plus">
                                +
                              </Button>
                              <Input
                                type="number"
                                ...
                                disabled
                              />
                              <Button onClick={updateFieldChanged(index)} value="minus">
                                -
                              </Button>
                            </ButtonGroup>
                          </TableCell>
                          <TableCell>{Number(key.cena).toFixed(2)}</TableCell>
                          <TableCell>{(Number(key.cena).toFixed(2) * key.skaits).toFixed(2)}</TableCell>
                          <TableCell>
                            <Button variant="outlined" onClick={updateFieldChanged(index)} value="delete">
                              Dzēst
                            </Button>
                          </TableCell>
                        </TableRow>
          ...
          <Container
            disableGutters
            maxWidth={false}
            sx={{ display: 'flex', justifyContent: 'space-around', p: '1rem 2rem', alignItems: 'center' }}
            component={Paper}
          >
